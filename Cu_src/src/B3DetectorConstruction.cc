...
/*
 //colimeter
  //
  G4double coli_inner_r=1.*mm;
  G4double coli_outer_r=5.*mm;
  G4double coli_z=10.*mm;

      G4Tubs* solidcoli =
      new G4Tubs("coli",                       //its name
         coli_inner_r,coli_outer_r,coli_z, 0., twopi); //its size
        
    G4LogicalVolume* logiccoli =
      new G4LogicalVolume(solidcoli,//its solid
                          target_mat1,         //its material
                          "colimeter");            //its name

    G4double phi0 = pi/2;
    G4RotationMatrix rotm0  = G4RotationMatrix();
    rotm0.rotateX(1*phi0);
    G4ThreeVector uz0 = G4ThreeVector(0., -1.*cm, 0.);
    G4ThreeVector position0 = uz0;//(0.5*coli_z)*uz0;
    G4Transform3D transform0 = G4Transform3D(rotm0,position0);
   
    G4VPhysicalVolume* physcoli =
      new G4PVPlacement(transform0,                     //no rotation
			//G4ThreeVector(0.*cm,-1.*cm,0*cm),    //at (0,0,0)
